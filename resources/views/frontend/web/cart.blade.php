                    ...
                    <div class="col-sm-12 table-responsive-xs">
                        <table class="table cart-table">
                            ...
                            @foreach($cart as $product)
                                ...
                                @endphp
                                <tr data-id="{{$product->id}}" data-price="{{$product->shopProduct->price}}">
                                    <td class="text-center">
                                        <a href="{{route('ware.show' , $product->shopProduct->slug)}}">
                                            @if(sizeof(json_decode($product->shopProduct->pictures)) > 0 )
                                                <img alt=""
                                                     src="{{asset('image/shop_product/'.$product->shopProduct->id.'/'.json_decode($product->shopProduct->pictures)[0]->picture)}}"/>
                                            @else
                                    ...
                                    <td class="text-center">
                                        <a class="font-weight-bold"
                                           href="{{route('ware.show' , $product->shopProduct->slug)}}">
                                        ...
                                        <div class="mobile-cart-content row">
                                            ...
                                            <div class="col">
                                                <h2 class="td-color">{{number_format($product->shopProduct->price , 0)}}</h2>
                                        ...
                                        <div class="qty-box">
                                            <div class="input-group">
                                                <input type="number" name="quantity"
                                                       class="form-control input-number cart-count"
                                                       min="1" max="{{$product->shopProduct->count}}"
                                                       value="{{$product->count}}">
                                            </div>
                                        </div>
                                    </td>
                                    <td class="text-center">
                                    <span
                                        class="td-color total_price">{{number_format($product->total() , 0)}}</span>
                                        <span class="td-color">&nbsp;ريال</span>
                                    </td>
                                    <td class="text-center">
                                    <span class="btn btn-outline-danger rounded-circle cart_item_delete"
                                          data-id="{{$product->id}}">
